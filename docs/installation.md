...
Dwi modules are plain Agent Skills. `SKILL.md` is the only required behavior file. There is no separate Dwi installer service, daemon, runtime package, MCP server, or website dependency. A supported AI coding tool such as Codex or Claude Code is still required.
